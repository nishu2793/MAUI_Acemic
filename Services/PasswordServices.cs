...
{
    public class PasswordServices : IPasswordRepository
    {
        public async Task<bool> Passmatch(string password, string did)
        {
            var Passmatch = new UserInfo()
            {
                Did = did,
                Password = password,
            };
            ...
            var json = JsonConvert.SerializeObject(Passmatch);
            var postData = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await httpClient.PostAsync("https://acemicapi.azurewebsites.net/api/UserRegisterTemp/SavePasswordTemp", postData);
            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                //var passtext = data.password;
                //Preferences.Set("PasswordKey", passtext);
